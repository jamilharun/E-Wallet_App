...
export default function DisplaySubs() {
  const getsubs = () => {
    const listofSubs = [{ id: 1 }, { id: 3 }, { id: 5 }];
  ...
  return (
    <div
      className="border-EWred border-4 rounded-lg space-y-2  overflow-y-auto flex flex-col transactcola 
          ...
          {sub.isSubscrived() ? (
            <div>
              <p className="text-end text-xl">
                ...
                {sub.currency ? sub.currency : "N/A"}
              </p>
              <div className="flex">
                <p className="text-xl xl:text-lg">
                  {sub.monthly ? sub.monthly : "N/A"}
                  <span className="text-sm ">
                    {sub.currency ? sub.currency : "N/A"}/m
                  </span>
                </p>
                <p className="text-xl xl:text-lg xl:hidden">
                  {sub.yearly ? sub.yearly : "N/A"}
                  <span className="text-sm xl:hidden">
                    {sub.currency ? sub.currency : "N/A"}/yrly
                  </span>
                </p>
              </div>
            </div>
          ) : (
            <p className="text-EWdarkBlue bg-EWred text-2xl xl:text-xl rounded-lg font-semibold xl:font-medium p-1">
              Follow
            </p>
          )}
        </div>
